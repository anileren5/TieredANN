...
// RecallAllMetrics and RecallHitMetrics are defined in qvcache/utils.h
...
// calculate_recall and calculate_hit_recall are now defined in qvcache/utils.h
// We use those definitions instead of local ones to avoid ambiguity
...
int main(int argc, char **argv) {
    ...
    if (data_type == "float") {
        std::unique_ptr<qvcache::BackendInterface<float, uint32_t>> backend = std::make_unique<qvcache::BruteforceBackend<float>>(data_path);
        ...
        std::unique_ptr<qvcache::BackendInterface<int8_t, uint32_t>> backend = std::make_unique<qvcache::BruteforceBackend<int8_t>>(data_path);
        ...
        std::unique_ptr<qvcache::BackendInterface<uint8_t, uint32_t>> backend = std::make_unique<qvcache::BruteforceBackend<uint8_t>>(data_path);
