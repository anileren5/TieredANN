...
void experiment_with_backend(
   ...
   qvcache::QVCache<T> qvcache(
    ...
    for (int i = 0; i < n_search_iter; i++) {
        std::vector<bool> hit_results = hybrid_search(qvcache, query, query_num, query_aligned_dim, K, memory_L, search_threads, query_result_tags, res, beamwidth, data_path);
