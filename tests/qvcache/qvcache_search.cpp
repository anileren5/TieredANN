...
void experiment_with_backend(
   ...
   qvcache::QVCache<T> qvcache(
    ...
    for (int i = 0; i < n_search_iter; i++) {
        auto [hit_results, hybrid_metrics] = hybrid_search(qvcache, query, query_num, query_aligned_dim, K, memory_L, search_threads, query_result_tags, res, beamwidth, data_path);
        RecallAllMetrics recall_all = calculate_recall<T, TagT>(K, groundtruth_ids, query_result_tags, query_num, groundtruth_dim); 
        RecallHitMetrics recall_hits = calculate_hit_recall<T, TagT>(K, groundtruth_ids, query_result_tags, hit_results, query_num, groundtruth_dim);
