...
// Greator header for set_sector_len (needed by QVCache)
...
std::pair<std::vector<bool>, HybridMetrics> hybrid_search(
    ...
    // Note: stats not used with SPTAG backend, but QVCache interface requires it
    void* stats = nullptr;
...
// Main experiment logic for window-based benchmark with SPTAG backend
...
void experiment_benchmark(
   ...
   std::unique_ptr<qvcache::BackendInterface<T, TagT>> sptag_backend = 
       std::make_unique<qvcache::SPTAGClientBackend<T, TagT>>(
           sptag_server_addr, sptag_server_port, vector_dim, data_path);
   
   // Create a dummy disk_index_prefix (QVCache requires it but SPTAG backend doesn't use it)
   std::string disk_index_prefix = "/tmp/dummy_index";
   
   qvcache::QVCache<T> qvcache(
       data_path, disk_index_prefix,
       0, memory_L, B, M, alpha,  // R=0 since SPTAG backend doesn't use it
       1, search_threads,  // build_threads=1 (not used with SPTAG)
       (bool)use_reconstructed_vectors,
       p, deviation_factor,
       memory_index_max_points,
       0,  // beamwidth=0 (not used with SPTAG)
       use_regional_theta,
       pca_dim,
       buckets_per_dim,
...
int main(int argc, char **argv) {
    ...
    try {
        ...
        desc.add_options()
            ...
            ("vector_dim", po::value<size_t>(&vector_dim)->default_value(0), "Vector dimension (0 = auto-detect from data_path)");
        po::variables_map vm;
