...
int main(int argc, char **argv) {
    ...
    if (data_type == "float") {
        ...
        experiment_benchmark<int8_t>(data_type, data_path, query_path, groundtruth_path, disk_index_prefix, R, disk_L, K, B, M, build_threads, search_threads, disk_index_already_built, beamwidth, n_splits, n_split_repeat, metric);
    } else if (data_type == "uint8") {
        experiment_benchmark<uint8_t>(data_type, data_path, query_path, groundtruth_path, disk_index_prefix, R, disk_L, K, B, M, build_threads, search_threads, disk_index_already_built, beamwidth, n_splits, n_split_repeat, metric);
    } else {
