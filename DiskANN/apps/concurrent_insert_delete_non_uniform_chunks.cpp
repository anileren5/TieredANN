...
void calculate_recall(const uint32_t K, TagT*& groundtruth_ids, std::vector<TagT>& query_result_tags, const uint32_t query_num, const uint32_t groundtruth_dim, uint32_t start, uint32_t end, size_t chunk_size) {
    ...
    for (int32_t i = 0; i < query_num; i++) {
        ...
        for (int32_t j = 0; j < K; j++) {
            calculated_closest_neighbors.insert(*(query_result_tags.data() + i * K + j) - (start));
...
void experiment(const std::string &data_path,
    ...
    build<T, TagT>(*index, chunk_sizes[1], build_threads, data, aligned_dim);
    ...
    calculate_recall<T>(K, groundtruth_ids, query_result_tags, query_num, groundtruth_dim, 0, chunk_sizes[1], chunk_sizes[1]);

    std::future<void> insert_task;
    std::future<void> delete_task;

    size_t inserted_so_far = chunk_sizes[1];

    for (size_t current_chunk = 2; current_chunk <= num_chunks; current_chunk++) {
        // Calculate batch boundaries
        size_t previous_chunk_size = chunk_sizes[current_chunk - 1];
        size_t current_chunk_size = chunk_sizes[current_chunk];

        size_t insert_start = inserted_so_far;
        size_t insert_end = inserted_so_far + current_chunk_size;

        size_t delete_start = inserted_so_far - previous_chunk_size;
        size_t delete_end = inserted_so_far;
        ...
        inserted_so_far += current_chunk_size;
        ...
        calculate_recall<T>(K, groundtruth_ids, query_result_tags, query_num, groundtruth_dim, insert_start, insert_end, chunk_sizes[current_chunk]);
    }
...
int main(int argc, char **argv) {
    ...
    if (data_type == std::string("int8"))
        experiment<int8_t>(data_path, query_path, chunks_groundtruth_path, R, L, K, alpha, build_threads, insert_threads, consolidate_threads, search_threads);
    else if (data_type == std::string("uint8"))
        experiment<uint8_t>(data_path, query_path, chunks_groundtruth_path, R, L, K, alpha, build_threads, insert_threads, consolidate_threads, search_threads);
    else if (data_type == std::string("float"))
        experiment<float>(data_path, query_path, chunks_groundtruth_path, R, L, K, alpha, build_threads, insert_threads, consolidate_threads, search_threads);
    else
