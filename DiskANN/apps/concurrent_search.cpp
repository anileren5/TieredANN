...
void experiment(const std::string &data_path,
    ...
    for (size_t current_chunk = 2; current_chunk <= num_chunks; current_chunk++) {
        ...
        // Only insert phase
        std::cout << "[" << get_current_timestamp() << "] " << "only_insert_phase_start()" << std::endl;
        ...
        delete_task = std::async(std::launch::async,
            [&index, delete_params, insert_start, insert_end]() {
                delete_and_consolidate<T, TagT>(*index, delete_params, insert_start, insert_end); // Delete the chunk that was just inserted
            });
        delete_task.wait(); // Wait for delete to finish
        
        std::cout << "[" << get_current_timestamp() << "] " << "concurrent_delete_phase_start()" << std::endl;
