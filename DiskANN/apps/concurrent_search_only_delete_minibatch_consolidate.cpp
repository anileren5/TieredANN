...
void experiment(const std::string &data_path,
    ...
    for (size_t current_chunk = 2; current_chunk <= num_chunks; current_chunk++) {
        ...
        size_t current_end = delete_start + minichunk_size;

        while (current_end < delete_end) {
            // Delete in chunks of minichunk_size
            delete_task = std::async(std::launch::async,
                [&index, delete_params, delete_start, current_end]() {
                    delete_and_consolidate<T, TagT>(*index, delete_params, delete_start, current_end);
                });
            delete_task.wait(); // Wait for delete to finish
            delete_start = current_end;
            current_end += minichunk_size;
        }
...
int main(int argc, char **argv) {
    ...
    uint32_t R, L, K, build_threads, insert_threads, consolidate_threads, search_threads, minichunk_size;
    float alpha;
    size_t chunk_size;
    ...
    try
    {
        ...
        desc.add_options()
            ...
            ("minichunk_size", po::value<uint32_t>(&minichunk_size)->required(), "Mini chunk size")
    ...
    if (data_type == std::string("int8"))
        experiment<int8_t>(data_path, query_path, chunks_groundtruth_path, chunk_size, R, L, K, alpha, build_threads, insert_threads, consolidate_threads, search_threads, minichunk_size);
    else if (data_type == std::string("uint8"))
        experiment<uint8_t>(data_path, query_path, chunks_groundtruth_path, chunk_size, R, L, K, alpha, build_threads, insert_threads, consolidate_threads, search_threads, minichunk_size);
    else if (data_type == std::string("float"))
        experiment<float>(data_path, query_path, chunks_groundtruth_path, chunk_size, R, L, K, alpha, build_threads, insert_threads, consolidate_threads, search_threads, minichunk_size);
    else
