...
# Script to run backend-only cache pressure experiment with noisy queries
# This script uses the noisy queries generated by generate_noisy_queries.py
# and runs the backend_cache_pressure experiment with window-based processing
# The experiment can be repeated n_round times to study backend behavior
...
N_ROUND=2 # Number of rounds to repeat the entire experiment
...
./build/tests/backend_cache_pressure \
  --data_type "$DATA_TYPE" \
  --data_path "$DATA_PATH" \
