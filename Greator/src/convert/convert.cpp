...
int aux_main(int argc, char** argv) {
  ...
  greator::cout << "Loading source vector from file " << argv[2] << std::endl;
  greator::load_bin<source_type>(argv[2], sourceVec, num_points, num_dims);

  dest_type* destVec = new dest_type[num_points * num_dims];
  greator::convert_types<source_type, dest_type>(sourceVec, destVec, num_points,
                                             num_dims);
  greator::cout << "Converted vector from type " << argv[1] << "to type " << argv[3]
            << std::endl;
  greator::save_bin<dest_type>(argv[4], destVec, num_points, num_dims);
  greator::cout << "Saved converted vector to file " << argv[4] << std::endl;
  return 0;
}
...
int main(int argc, char** argv) {
  ...
  if (sourceType == destType) {
    greator::cout << "Nothing to do. source and destination types are the same."
              << std::endl;
    return 0;
  }
  ...
  if (sourceType == "int8" && destType == "uint8") {
    ...
    greator::cout << "One of the source or destination types is unknown: "
