...
void gen_random_slice(const std::string base_file,
  ...
  greator::cout << "Loading base " << base_file << ". #points: " << npts_u32
                << ". #dim: " << nd_u32 << "." << std::endl;
  ...
  greator::cout << "Wrote " << num_sampled_pts_u32
...
int generate_pq_pivots(const float *passed_train_data, size_t num_train,
  ...
  if (num_pq_chunks > dim) {
    greator::cerr << " Error: number of chunks more than dimension"
                  << std::endl;
    return -1;
  }
  ...
  for (uint64_t i = 0; i < num_train; i++) {
    for (uint64_t j = 0; j < dim; j++) {
      if (passed_train_data[i * dim + j] != train_data[i * dim + j])
        greator::cerr << "error in copy" << std::endl;
    }
  }
...
#pragma omp parallel for schedule(static, 65536)
  ...
  greator::cout << "Kmeans time: " << kmeans_time
  ...
  cumul_bytes[1] =
      cumul_bytes[0] +
      greator::save_bin<float>(pq_pivots_path.c_str(), full_pivot_data.get(),
                               (size_t)num_centers, dim, cumul_bytes[0]);
  cumul_bytes[2] = cumul_bytes[1] + greator::save_bin<float>(
                                        pq_pivots_path.c_str(), centroid.get(),
                                        (size_t)dim, 1, cumul_bytes[1]);
  cumul_bytes[3] =
      cumul_bytes[2] +
      greator::save_bin<uint32_t>(pq_pivots_path.c_str(), rearrangement.data(),
                                  rearrangement.size(), 1, cumul_bytes[2]);
  cumul_bytes[4] =
      cumul_bytes[3] +
      greator::save_bin<uint32_t>(pq_pivots_path.c_str(), chunk_offsets.data(),
                                  chunk_offsets.size(), 1, cumul_bytes[3]);
  greator::save_bin<_u64>(pq_pivots_path.c_str(), cumul_bytes.data(),
                          cumul_bytes.size(), 1, 0);

  greator::cout << "Saved pq pivot data to " << pq_pivots_path << " of size "
...
int generate_pq_data_from_pivots(const std::string data_file,
  ...
  if (!file_exists(pq_pivots_path)) {
    greator::cout << "ERROR: PQ k-means pivot file not found" << std::endl;
    throw greator::ANNException("PQ k-means pivot file not found", -1);
  } else {
    _u64 nr, nc;
    std::unique_ptr<_u64[]> file_offset_data;

    greator::load_bin<_u64>(pq_pivots_path.c_str(), file_offset_data, nr, nc,
                            0);

    if (nr != 5) {
      greator::cout << "Error reading pq_pivots file " << pq_pivots_path
    ...
    greator::load_bin<float>(pq_pivots_path.c_str(), full_pivot_data, nr, nc,
                             file_offset_data[0]);
    ...
    greator::load_bin<float>(pq_pivots_path.c_str(), centroid, nr, nc,
                             file_offset_data[1]);
    ...
    greator::load_bin<uint32_t>(pq_pivots_path.c_str(), rearrangement, nr, nc,
                                file_offset_data[2]);
    ...
    greator::load_bin<uint32_t>(pq_pivots_path.c_str(), chunk_offsets, nr, nc,
                                file_offset_data[3]);
    ...
    greator::cout << "Loaded PQ pivot information" << std::endl;
  }
  ...
  for (size_t block = 0; block < num_blocks; block++) {
    ...
    greator::convert_types<T, float>(block_data_T.get(), block_data_tmp.get(),
                                     cur_blk_size, dim);
...
#pragma omp parallel for schedule(static, 8192)
    ...
    if (num_centers > 256) {
      ...
      greator::convert_types<uint32_t, uint8_t>(
...
int estimate_cluster_sizes(const std::string data_file, float *pivots,
  ...
  if (test_dim != dim) {
    greator::cout << "Error. dimensions dont match for pivot set and base set"
                  << std::endl;
    return -1;
  }
  ...
  greator::get_bin_metadata(data_file, num_points, num_dim);
  size_t block_size = num_points <= BLOCK_SIZE ? num_points : BLOCK_SIZE;
...
int shard_data_into_clusters(const std::string data_file, float *pivots,
  ...
  size_t num_points = npts32;
  if (basedim32 != dim) {
    greator::cout << "Error. dimensions dont match for train set and base set"
                  << std::endl;
    return -1;
  }
  ...
  for (size_t block = 0; block < num_blocks; block++) {
    ...
    base_reader.read((char *)block_data_T.get(),
                     sizeof(T) * (cur_blk_size * dim));
    greator::convert_types<T, float>(block_data_T.get(), block_data_float.get(),
                                     cur_blk_size, dim);
  ...
  greator::cout << "Actual shard sizes: " << std::flush;
  for (size_t i = 0; i < num_centers; i++) {
    _u32 cur_shard_count = (_u32)shard_counts[i];
    total_count += cur_shard_count;
    greator::cout << cur_shard_count << " ";
...
int partition(const std::string data_file, const float sampling_rate,
  ...
  greator::cout << "Processing global k-means (kmeans_partitioning Step)"
                << std::endl;
  ...
  greator::cout << "Saving global k-center pivots" << std::endl;
  greator::save_bin<float>(output_file.c_str(), pivot_data, (size_t)num_parts,
                           train_dim);
...
int partition_with_ram_budget(const std::string data_file,
  ...
  while (!fit_in_ram) {
    ...
    greator::cout << "With " << num_parts << " parts, max estimated RAM usage: "
...
template DISKANN_DLLEXPORT int
partition<int8_t>(const std::string data_file, const float sampling_rate,
                  size_t num_centers, size_t max_k_means_reps,
                  const std::string prefix_path, size_t k_base);
template DISKANN_DLLEXPORT int
partition<uint8_t>(const std::string data_file, const float sampling_rate,
                   size_t num_centers, size_t max_k_means_reps,
                   const std::string prefix_path, size_t k_base);
template DISKANN_DLLEXPORT int
partition<float>(const std::string data_file, const float sampling_rate,
                 size_t num_centers, size_t max_k_means_reps,
                 const std::string prefix_path, size_t k_base);
...
template DISKANN_DLLEXPORT int
generate_pq_pivots<float>(const std::unique_ptr<float[]> &passed_train_data,
                          size_t num_train, unsigned dim, unsigned num_centers,
                          unsigned num_pq_chunks, unsigned max_k_means_reps,
                          std::string pq_pivots_path);
template DISKANN_DLLEXPORT int
generate_pq_pivots<int8_t>(const std::unique_ptr<int8_t[]> &passed_train_data,
                           size_t num_train, unsigned dim, unsigned num_centers,
                           unsigned num_pq_chunks, unsigned max_k_means_reps,
                           std::string pq_pivots_path);
