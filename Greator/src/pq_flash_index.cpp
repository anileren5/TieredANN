...
void pq_dist_lookup(const _u8 *pq_ids, const _u64 n_pts, const _u64 pq_nchunks,
...
PQFlashIndex<T, TagT>::PQFlashIndex(
    greator::Metric m, std::shared_ptr<AlignedFileReader> &fileReader,
  ...
  if (m == greator::Metric::COSINE) {
    if (std::is_floating_point<T>::value) {
      greator::cout << "Cosine metric chosen for (normalized) float data."
      ...
      greator::cerr << "WARNING: Cannot normalize integral data types."
  ...
  this->dist_cmp.reset(greator::get_distance_function<T>(m));
  this->dist_cmp_float.reset(greator::get_distance_function<float>(m));
}

template <typename T, typename TagT> PQFlashIndex<T, TagT>::~PQFlashIndex() {
  Timer start;
  ...
  greator::cout << "Thread Data size: " << this->thread_data.size() << "\n";
  ...
  if (nhood_cache_buf != nullptr) {
    ...
    greator::aligned_free(coord_cache_buf);
  }
...
void PQFlashIndex<T, TagT>::setup_thread_data(_u64 nthreads) {
  greator::cout << "Setting up thread-specific contexts for nthreads: "
                << nthreads << "\n";
...
#pragma omp critical
    {
      ...
      greator::alloc_aligned((void **)&scratch.coord_scratch, coord_alloc_size,
                             256);
      greator::alloc_aligned((void **)&scratch.sector_scratch,
                             MAX_N_SECTOR_READS * SECTOR_LEN, SECTOR_LEN);
      greator::alloc_aligned((void **)&scratch.aligned_scratch,
                             256 * sizeof(float), 256);
      greator::alloc_aligned((void **)&scratch.aligned_pq_coord_scratch,
                             32768 * 32 * sizeof(_u8), 256);
      greator::alloc_aligned((void **)&scratch.aligned_pqtable_dist_scratch,
                             25600 * sizeof(float), 256);
      greator::alloc_aligned((void **)&scratch.aligned_dist_scratch,
                             512 * sizeof(float), 256);
      greator::alloc_aligned((void **)&scratch.aligned_query_T,
                             this->aligned_dim * sizeof(T), 8 * sizeof(T));
      greator::alloc_aligned((void **)&scratch.aligned_query_float,
...
void PQFlashIndex<T, TagT>::destroy_thread_data() {
  for (auto &data : this->thread_data_backing_buf) {
    ...
    greator::aligned_free((void *)scratch.coord_scratch);
    greator::aligned_free((void *)scratch.sector_scratch);
    greator::aligned_free((void *)scratch.aligned_scratch);
    greator::aligned_free((void *)scratch.aligned_pq_coord_scratch);
    greator::aligned_free((void *)scratch.aligned_pqtable_dist_scratch);
    greator::aligned_free((void *)scratch.aligned_dist_scratch);
    greator::aligned_free((void *)scratch.aligned_query_float);
    greator::aligned_free((void *)scratch.aligned_query_T);
  }
  this->reader->deregister_all_threads();
}

template <typename T, typename TagT>
void PQFlashIndex<T, TagT>::load_cache_list(std::vector<uint32_t> &node_list) {
  ...
  greator::alloc_aligned((void **)&coord_cache_buf,
...
void PQFlashIndex<T, TagT>::generate_cache_list_from_sample_queries(
  ...
  if (file_exists(sample_bin)) {
    greator::load_aligned_bin<T>(sample_bin, samples, sample_num, sample_dim,
                                 sample_aligned_dim);
  }
#endif
  else {
    greator::cerr << "Sample bin file not found. Not generating cache."
                  << std::endl;
    return;
  }
...
#pragma omp parallel for schedule(dynamic, 1) num_threads(nthreads)
  ...
  greator::aligned_free(samples);
}

template <typename T, typename TagT>
void PQFlashIndex<T, TagT>::cache_bfs_levels(_u64 num_nodes_to_cache,
                                             std::vector<uint32_t> &node_list) {

  // NOTE: This function causes division by zero, for the details look at the
  // next NOTE in this function.
  ...
  if (num_nodes_to_cache > tenp_nodes) {
    greator::cout << "Reducing nodes to cache from: " << num_nodes_to_cache
                  << " to: " << tenp_nodes
                  << "(10 percent of total nodes:" << this->num_points << ")"
                  << std::endl;
    num_nodes_to_cache = tenp_nodes == 0 ? 1 : tenp_nodes;
  }
  greator::cout << "Caching " << num_nodes_to_cache << "..." << std::endl;
  ...
  while ((node_list.size() + cur_level->size() < num_nodes_to_cache) &&
    ...
    for (size_t block = 0; block < nblocks && !finish_flag; block++) {
      ...
      for (size_t cur_pt = start; cur_pt < end; cur_pt++) {
        ...
        read.len = SECTOR_LEN;
        read.buf = buf;
        // NOTE: This nnodes_per_sector causes division by zero!
    ...
    greator::cout << ". #nodes: " << node_list.size() - prev_node_list_size
...
void PQFlashIndex<T, TagT>::use_medoids_data_as_centroids() {
  ...
  greator::cout << "Loading centroid data from medoids vector data of "
...
int PQFlashIndex<T, TagT>::load(const char *index_prefix, _u32 num_threads,
  ...
  if (new_index_format) {
    ...
    greator::cout << "Disk-Index File Meta-data: " << "# nodes per sector: "
                  << nnodes_per_sector
    ...
    if (this->num_frozen_points == 1) {
      ...
      // if (this->num_frozen_points == 1) {
      greator::cout << " Detected frozen point in index at location "
    ...
    greator::cout << "Tags offset: " << tags_offset
    ...
    if (actual_index_size != expected_file_size) {
      greator::cout << "File size mismatch for " << disk_index_file
    ...
    greator::cout << "Disk-Index File Meta-data: ";
    greator::cout << "# nodes per sector: " << nnodes_per_sector;
    greator::cout << ", max node len (bytes): " << max_node_len;
    greator::cout << ", max node degree: " << max_degree << std::endl;
    pagesize = (disk_nnodes + nnodes_per_sector - 1) / (nnodes_per_sector);
  ...
  greator::cout << "After single file index check, Tags offset: " << tags_offset
                << " PQ Pivots offset: " << pq_pivots_offset
                << " PQ Vectors offset: " << pq_vectors_offset << std::endl;
...
#else
  greator::load_bin<_u8>(pq_compressed_vectors, this->data, npts_u64,
                         nchunks_u64, pq_vectors_offset);
#endif
  this->num_points = npts_u64;
  this->n_chunks = nchunks_u64;

  greator::cout << "Load compressed vectors from file: "
  ...
  if (disk_nnodes != num_points) {
    greator::cout << "Mismatch in #points for compressed data file and disk "
                     "index file: "
...
// read index metadata
  ...
  if (file_exists(medoids_file)) {
    size_t tmp_dim;
    greator::load_bin<uint32_t>(medoids_file, medoids, num_medoids, tmp_dim);
#endif
    ...
    if (!file_exists(centroids_file)) {
...
#else
      greator::load_aligned_bin<float>(centroids_file, centroid_data,
                                       num_centroids, tmp_dim, aligned_tmp_dim);
#endif
      if (aligned_tmp_dim != aligned_dim || num_centroids != num_medoids) {
        ...
        greator::cerr << stream.str() << std::endl;
        throw greator::ANNException(stream.str(), -1, __FUNCSIG__, __FILE__,
                                    __LINE__);
      }
    }
  ...
  // load tags
  if (this->enable_tags) {
    greator::cout << "Loading tags...";
    if (!this->single_index_file) {
      std::string tag_file = disk_index_file;
      tag_file = tag_file + ".tags";
      greator::cout << "from " << tag_file << std::endl;
      this->load_tags(tag_file);
    } else {
      greator::cout << "from " << disk_index_file << std::endl;
      this->load_tags(disk_index_file, tags_offset);
    }
  }
  greator::cout << "PQFlashIndex loaded successfully." << std::endl;
  return 0;
}

template <typename T, typename TagT> _u64 PQFlashIndex<T, TagT>::return_nd() {
...
size_t PQFlashIndex<T, TagT>::cached_beam_search_vectors(
  ...
  for (uint32_t i = 0; i < l_search && res_count < k_search; i++) {
    ...
    if (res_tags != nullptr && this->tags != nullptr) {
      ...
      throw greator::ANNException("Could not find tags", -1, __FUNCSIG__,
                                  __FILE__, __LINE__);
    }
    res_count++;
  }
...
size_t PQFlashIndex<T, TagT>::cached_beam_search(
  ...
  // Copy top-k_search IDs into res_tags and distances
  for (_u64 i = 0; i < std::min(k_search, (_u64)expanded_nodes_info.size());
       ++i) {
    res_tags[i] = expanded_nodes_info[i].id;
...
size_t PQFlashIndex<T, TagT>::cached_beam_search_ids(
    const T *query, const _u64 k_search, const _u64 l_search, uint64_t *res_ids,
    float *distances, const _u64 beam_width, QueryStats *stats) {
  // iterate to fixed point
  std::vector<Neighbor> expanded_nodes_info;
  expanded_nodes_info.reserve(2 * l_search);
  ...
  while (k < cur_list_size && num_ios < IO_LIMIT) {
    ...
    if (!frontier.empty()) {
      ...
      for (_u64 i = 0; i < frontier.size(); i++) {
        ...
        // TODO: This division by zero error occurs here too!
        uint32_t pageid = 1 + id / this->nnodes_per_sector;
        lock_pageid.push_back(pageid);
        frontier_read_reqs.emplace_back(NODE_SECTOR_NO(((size_t)id)) *
                                            SECTOR_LEN,
    ...
    for (auto &frontier_nhood : frontier_nhoods) {
      ...
      if (exclude_nodes != nullptr) {
        exclude_cur_node =
            (exclude_nodes->find(frontier_nhood.first) != exclude_nodes->end());
      }
...
#pragma omp parallel for schedule(dynamic, chunk_size) num_threads(10)
  for (int i = 0; i < write_req.size(); i++) {
    ...
    CASRWLock *pmutex = &pagemutex[(1 + start_id[i] / this->nnodes_per_sector)];
    pmutex->WriteLock();
  }
...
void PQFlashIndex<T, TagT>::reload_index(
  ...
  greator::cout << "RELOAD: Loading compressed vectors from "
                << pq_compressed_vectors << "\n";
  greator::load_bin<_u8>(pq_compressed_vectors, data, npts_u64, nchunks_u64);
  ...
  greator::cout << "RELOAD: Loading graph from " << disk_index_file << "\n";
  ...
  greator::cout << "Medoid-ID: " << medoid_id_on_file << "\n";
  ...
  uint32_t node_cache_count =
      std::min((_u32)this->nhood_cache.size(), (_u32)this->coord_cache.size());

  // clear cache
  greator::cout << "RELOAD: Clearing cache.\n";
  ...
  // refresh cache
  greator::cout << "RELOAD: Caching " << node_cache_count << " nodes.\n";
  std::vector<uint32_t> node_list;
...
void PQFlashIndex<T, TagT>::load_tags(const std::string &tag_file_name,
  ...
  if (file_exists(tag_file_name)) {
    greator::cout << "Loading tags from " << tag_file_name << std::endl;
    size_t tag_num, tag_dim;
    greator::load_bin<TagT>(tag_file_name, tags, tag_num, tag_dim, offset);
  } else {
    greator::cout << "Tags file " << tag_file_name << " does not exist. "
                  << std::endl;
  }
}

template <typename T, typename TagT>
void PQFlashIndex<T, TagT>::get_active_tags(tsl::robin_set<TagT> &active_tags) {
  active_tags.clear();
  if (this->enable_tags) {
...
int PQFlashIndex<T, TagT>::get_vector_by_tag(const TagT &tag,
                                             T *vector_coords) {
  if (!enable_tags) {
    greator::cout << "Tags are disabled, cannot retrieve vector" << std::endl;
    return -1;
  }
  size_t pos;
  ...
  if (!tag_found) {
    greator::cout << "Tag not found." << std::endl;
    return -2;
  }
