...
void rotate_data_randomly(float *data, size_t num_points, size_t dim,
                          float *rot_mat, float *&new_mat, bool transpose_rot) {
  CBLAS_TRANSPOSE transpose = CblasNoTrans;
  if (transpose_rot) {
    greator::cout << "Transposing rotation matrix.." << std::flush;
    transpose = CblasTrans;
  }
  greator::cout << "done Rotating data with random matrix.." << std::flush;
...
void compute_closest_centers_in_block(
  ...
  if (k > num_centers) {
    greator::cout << "ERROR: k (" << k << ") > num_center(" << num_centers
                  << ")" << std::endl;
    return;
  }
  ...
  cblas_sgemm(CblasRowMajor, CblasNoTrans, CblasTrans, (MKL_INT)num_points,
              (MKL_INT)num_centers, (MKL_INT)1, 1.0f, docs_l2sq, (MKL_INT)1,
              ones_a, (MKL_INT)1, 0.0f, dist_matrix, (MKL_INT)num_centers);
  ...
  cblas_sgemm(CblasRowMajor, CblasNoTrans, CblasTrans, (MKL_INT)num_points,
              ...
              centers, (MKL_INT)dim, 1.0f, dist_matrix, (MKL_INT)num_centers);
...
void compute_closest_centers(float *data, size_t num_points, size_t dim,
  ...
  for (size_t cur_blk = 0; cur_blk < N_BLOCKS; cur_blk++) {
    ...
    math_utils::compute_closest_centers_in_block(
        data_cur_blk, num_pts_blk, dim, pivot_data, num_centers, pts_norms_blk,
        pivs_norms_squared, closest_centers, distance_matrix, k);

#pragma omp parallel for schedule(static, 1)
    for (int64_t j = cur_blk * PAR_BLOCK_SIZE;
         j < std::min((_s64)num_points, (_s64)((cur_blk + 1) * PAR_BLOCK_SIZE));
         j++) {
    ...
    for (int64_t chunk = 0; chunk < (_s64)nchunks; ++chunk)
      for (size_t d = chunk * CHUNK_SIZE;
        ...
        residuals[chunk * BUF_PAD] += math_utils::calc_distance(
            data + (d * dim), centers + (size_t)closest_center[d] * (size_t)dim,
            dim);
...
void kmeanspp_selecting_pivots(float *data, size_t num_points, size_t dim,
                               float *pivot_data, size_t num_centers) {
  if (num_points > 1 << 23) {
    greator::cout << "ERROR: n_pts " << num_points
