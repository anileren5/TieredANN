...
template <typename T> struct DiskNode {
  uint32_t id = 0;
  T *coords = nullptr;
  uint32_t nnbrs;
  uint32_t *nbrs;
...
template <typename T, typename TagT = uint32_t> class PQFlashIndex {
public:
  ...
  DISKANN_DLLEXPORT PQFlashIndex(greator::Metric m,
  ...
  DISKANN_DLLEXPORT size_t cached_beam_search_vectors(
      const T *query, const _u64 k_search, const _u64 l_search, TagT *res_tags,
      float *res_dists, std::vector<T *> &res_vectors, const _u64 beam_width,
      QueryStats *stats = nullptr);
  ...
  merge_write_through_diff_pages(std::vector<_u32> &start_id, char *scratch);
  ...
  DISKANN_DLLEXPORT void
  scan_deleted_nodes(const tsl::robin_set<uint32_t> &delete_set,
                     std::vector<DiskNode<T>> &deleted_nodes, char *buf,
                     char *backing_buf, const uint32_t sectors_per_scan,
                     bool id_map = false);
  ...
  DISKANN_DLLEXPORT TagT *get_tags() { return this->tags; }

  DISKANN_DLLEXPORT _u64 get_num_frozen_points() {
    return this->num_frozen_points;
  }

  DISKANN_DLLEXPORT _u64 get_frozen_loc() { return this->frozen_location; }
