...
namespace greator {
template <typename T> class FixedChunkPQTable {
...
#else
    greator::load_bin<float>(pq_pivots_path.c_str(), tables, nr, nc);
#endif
...
//_rearrangement_perm.bin
#ifdef EXEC_ENV_OLS
    diskann::load_bin<float>(files, pq_pivots_path + "_centroid.bin", centroid,
                             nr, nc);
#else
    greator::load_bin<float>(pq_pivots_path + "_centroid.bin", centroid, nr,
                             nc);
#endif

    if ((nr != this->ndims) || (nc != 1)) {
      greator::cerr << "Error reading centroids from pq_pivots file "
    ...
    diskann::load_bin<uint32_t>(files,
                                ...
                                rearrangement, nr, nc);
#else
    greator::load_bin<uint32_t>(pq_pivots_path + "_rearrangement_perm.bin",
                                rearrangement, nr, nc);
#endif
    if ((nr != this->ndims) || (nc != 1)) {
      greator::cerr << "Error reading re-arrangement data pq_pivots file "
                    << pq_pivots_path << ". file_dim  = " << nr
                    << ", file_cols = " << nc << " but expecting "
                    << this->ndims << " entries in 1 dimension.";
      throw greator::ANNException(
    ...
    diskann::load_bin<uint32_t>(files, pq_pivots_path + "_chunk_offsets.bin",
                                chunk_offsets, nr, nc);
#else
    greator::load_bin<uint32_t>(pq_pivots_path + "_chunk_offsets.bin",
                                chunk_offsets, nr, nc);
#endif
    ...
    if (nr != (uint64_t)num_chunks + 1 || nc != 1) {
      greator::cerr
          ...
          << ",nc=" << nc << ", expecting nr=" << num_chunks + 1 << ", nc=1."
          << std::endl;
      throw greator::ANNException(
    ...
    greator::cout << "Loaded PQ Pivots: #ctrs: " << NUM_PQ_CENTROIDS
                  << ", #dims: " << this->ndims
  ...
  void load_pq_pivots_new(MemoryMappedFiles &files,
                          const std::string &pq_pivots_path, size_t num_chunks,
                          size_t offset) {
#else
  void load_pq_pivots_new(const std::string &pq_pivots_path, size_t num_chunks,
                          size_t offset) {
#endif
    ...
    greator::load_bin<_u64>(pq_pivots_path, file_offset_data, nr, nc, offset);
#endif
    ...
    greator::cout << "Offsets: " << file_offset_data[0] << " "
...
#ifdef EXEC_ENV_OLS
  void load_pq_centroid_bin(MemoryMappedFiles &files, const char *pq_table_file,
                            size_t num_chunks, size_t offset = 0){
#else
  ...
  greator::cout << "Finished optimizing for PQ-PQ distance compuation "
                << std::endl;
  ...
  for (_u64 chunk = 0; chunk < n_chunks; chunk++) {
    ...
    for (_u64 j = chunk_offsets[chunk]; j < chunk_offsets[chunk + 1]; j++) {
      ...
      for (_u64 idx = 0; idx < 256; idx++) {
        ...
        double diff = centers_dim_vec[idx] - (query_vec[permuted_dim_in_query] -
                                              centroid[permuted_dim_in_query]);
...
void compute_distances(const _u8 *comp_src, const _u8 *comp_dsts, float *dists,
                       const _u32 count) {
...
void deflate_vec(const float *fp_vec, _u8 *out_pq_vec) {
  ...
  for (_u32 c = 0; c < n_chunks; c++) {
    ...
    for (_u32 i = 0; i < 256; i++) {
      float cur_dist = 0;
      for (_u64 d = chunk_offsets[c]; d < chunk_offsets[c + 1]; d++) {
        float diff = (tables[i * ndims + d] - ((float)fp_vec[rearrangement[d]] -
