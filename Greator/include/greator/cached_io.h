...
public:
  ...
  void open(const std::string &filename, uint64_t cacheSize,
    ...
    greator::cout << "Opened: " << filename.c_str() << ", size: " << fsize
    ...
    if (n_bytes <= (cache_size - cur_off)) {
      ...
      if (n_bytes - cached_bytes > fsize - reader.tellg()) {
        ...
        stream << "n_bytes: " << n_bytes << " cached_bytes: " << cached_bytes
               ...
               << std::endl;
        greator::cout << stream.str() << std::endl;
...
public:
  ...
  void open(const std::string &filename, uint64_t cache_size,
    ...
    greator::cout << "Opened: " << filename.c_str()
  ...
  void close() {
    ...
    if (writer.is_open())
      writer.close();
    greator::cout << "Finished writing " << fsize << "B" << std::endl;
  }
