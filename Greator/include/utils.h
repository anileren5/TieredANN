...
inline bool file_exists(const std::string &name, bool dirCheck = false) {
  ...
  greator::cout << " Stat(" << name.c_str() << ") returned: " << val
                << std::endl;
  if (val != 0) {
    switch (errno) {
    case EINVAL:
      greator::cout << "Invalid argument passed to stat()" << std::endl;
      break;
    case ENOENT:
      greator::cout << "File " << name.c_str() << " does not exist"
                    << std::endl;
      break;
    default:
      greator::cout << "Unexpected error in stat():" << errno << std::endl;
      break;
    }
    return false;
  } else {
...
inline void open_file_to_write(std::ofstream &writer,
  ...
  if (writer.fail()) {
    greator::cerr << std::string("Failed to open file") + filename +
                         " for write because "
...
inline _u64 get_file_size(const std::string &fname) {
  ...
  if (!reader.fail() && reader.is_open()) {
    ...
    greator::cerr << "Could not open file: " << fname << std::endl;
    return 0;
  }
}
...
DISKANN_DLLEXPORT float
calc_recall_set_tags(unsigned num_queries, unsigned *gold_std, unsigned dim_gs,
                     unsigned *our_results_tags, unsigned dim_or,
                     unsigned recall_at, unsigned subset_size,
                     std::string gt_tag_filename,
...
inline void load_bin_impl(std::basic_istream<char> &reader, T *&data,
  ...
  greator::cout << "Metadata: #pts = " << npts << ", #dims = " << dim << "..."
                << std::endl;
...
inline void load_bin(const std::string &bin_file, T *&data, size_t &npts,
  ...
  // END OLS
  greator::cout << "Reading bin file " << bin_file.c_str() << " ..."
                << std::endl;
...
inline void load_truthset(const std::string &bin_file, uint32_t *&ids,
  ...
  greator::cout << "Reading truthset file " << bin_file.c_str() << "..."
                << std::endl;
...
inline void load_aligned_bin_impl(std::basic_istream<char> &reader, T *&data,
  ...
  greator::cout << " Allocated " << allocSize << "bytes and copied data "
                << std::endl;
}
...
inline void load_aligned_bin(const std::string &bin_file, T *&data,
                             size_t &npts, size_t &dim, size_t &rounded_dim,
                             size_t offset = 0) {
  greator::cout << "Reading bin file " << bin_file << " at offset " << offset
                << "..." << std::flush;
  std::ifstream reader(bin_file, std::ios::binary | std::ios::ate);
  reader.seekg(0);
...
inline uint64_t save_data_in_base_dimensions(const std::string &filename,
                                             T *data, size_t npts, size_t ndims,
                                             size_t aligned_dim,
                                             size_t offset = 0) {
...
inline void copy_aligned_data_from_file(const std::string bin_file, T *&data,
                                        ...
                                        size_t offset = 0) {
  if (data == nullptr) {
    greator::cout << "Memory was not allocated for " << data
...
inline bool validate_file_size(const std::string &name) {
  ...
  if (actual_file_size != expected_file_size) {
    greator::cerr << "Error loading" << name
                  << ". Expected "
                     "size (metadata): "
                  << expected_file_size
...
template <typename T>
greator::Distance<T> *get_distance_function(greator::Metric m);
