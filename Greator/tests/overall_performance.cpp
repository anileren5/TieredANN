...
inline uint64_t save_bin_test(const std::string &filename, T *id, float *dist,
  ...
  greator::cout << "Writing bin: " << filename.c_str() << std::endl;
  writer.seekp(offset, writer.beg);
  int npts_i32 = (int)npts, ndims_i32 = (int)ndims;
  size_t bytes_written = npts * ndims * sizeof(T) + 2 * sizeof(uint32_t);
  writer.write((char *)&npts_i32, sizeof(int));
  writer.write((char *)&ndims_i32, sizeof(int));
  greator::cout << "bin: #pts = " << npts << ", #dims = " << ndims
  ...
  writer.close();
  greator::cout << "Finished writing bin." << std::endl;
...
void sync_search_kernel(T *query, size_t query_num, size_t query_aligned_dim,
                        ...
                        greator::MergeInsert<T, TagT> &sync_index,
  ...
  if (calRecall) {
    ...
    greator::load_truthset(truthset_file, gt_ids, gt_dists, gt_num, gt_dim,
                           &gt_tags);
...
#pragma omp parallel for num_threads(NUM_SEARCH_THREADS)
  ...
  int current_time = globalTimer.elapsed() / 1.0e6f - begin_time;
  if (calRecall) {
    recall = greator::calculate_recall(query_num, gt_ids, gt_dists, gt_dim,
...
template <typename T, typename TagT>
void merge_kernel(greator::MergeInsert<T, TagT> &sync_index,
                  std::string &save_path, uint32_t id_map = 0) {
  greator::Timer merget;
...
template <typename T, typename TagT>
void deletion_kernel(T *data_load, greator::MergeInsert<T, TagT> &sync_index,
...
#pragma omp parallel for num_threads(NUM_DELETE_THREADS)
  ...
  std::cout << "Mem index deletion time : " << timer.elapsed() / 1000 << " ms"
            ...
            << delete_latencies[(size_t)(0.999 * ((double)npts))] << " microsec"
            << std::endl;*/
}

template <typename T, typename TagT>
void insertion_kernel(T *data_load, greator::MergeInsert<T, TagT> &sync_index,
...
#pragma omp parallel for num_threads(NUM_INSERT_THREADS)
  ...
  std::cout << "Mem index insertion time : " << timer.elapsed() / 1000 << " ms"
            ...
            << insert_latencies[(size_t)(0.999 * ((double)npts))] << " microsec"
            << std::endl;*/
}
...
void get_trace(tsl::robin_set<uint32_t> &inactive_set,
  ...
  greator::cout << "Reading trace vectors from bin file " << data_path << "..."
                << std::flush;
  ...
  greator::alloc_aligned(((void **)&data_load), allocSize, 8 * sizeof(T));
...
void update(const std::string &data_path, const unsigned L_mem,
            ...
            std::string &trace_file_prefix, greator::Distance<T> *dist_cmp,
            uint32_t id_map = 0) {
  greator::Parameters paras;
  ...
  greator::Timer timer;
  // dim = 128;
  greator::load_aligned_bin<T>(data_path.c_str(), data_load, num_points, dim,
                               aligned_dim);
  std::cout << "Loaded full data for driver: (" << num_points << "," << dim
            << ") vectors." << std::endl;

  greator::Metric metric = greator::Metric::L2;
  greator::MergeInsert<T, TagT> sync_index(paras, dim, save_path + "_mem",
  ...
  greator::load_aligned_bin<T>(query_file, query, query_num, query_dim,
                               query_aligned_dim);
  ...
  for (int i = 0; i < batch; i++) {
    ...
    std::cout
        << "_________________________Over_insertion_kernel________________";

    merge_kernel(sync_index, save_path, id_map);
...
void build(const std::string &data_path, const unsigned L_mem,
  ...
  greator::save_bin<TagT>(tag_file, tags.data(), num_start, 1);
  delete[] data_load;
}
...
int main(int argc, char **argv) {
  if (argc < 14) {
    std::cout << "Correct usage: " << argv[0]
              ...
              << " <Lsearch> <#beam_width>" << " <trace_file_prefix>"
              << " <step>" << std::endl;
    exit(-1);
  }
  ...
  if (updateIndex) {
    if (std::string(argv[1]) == std::string("int8")) {
      greator::DistanceL2Int8 dist_cmp;
      ...
      greator::DistanceL2UInt8 dist_cmp;
