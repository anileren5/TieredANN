...
void search_kernel(
    ...
    std::vector<_u64> Lvec, greator::Index<T, TagT> &index,
  ...
  greator::cout << std::setw(4) << "Ls" << std::setw(12) << "QPS "
...
#pragma omp parallel for
    ...
    greator::cout << std::setw(4) << L << std::setw(12) << qps << std::setw(18)
...
int build_incremental_index(const std::string &data_path,
  ...
  greator::Index<T, TagT> index(greator::Metric::L2, dim, num_points + 100, 1,
                                false, true, 0);

  auto tag_path = memory_index_file + ".tags";
  index.load(memory_index_file.c_str());
  greator::cout << "Loaded index and tags and data" << std::endl;
  T *query = NULL;
  size_t query_num, query_dim, query_aligned_dim;
  greator::load_aligned_bin<T>(query_file, query, query_num, query_dim,
                               query_aligned_dim);
  greator::cout << "Search on static index" << std::endl;
  ...
  while (i < num_cycles) {
...

    {
      ...
      /*
            ...
            */
      greator::Timer del_timer;
      greator::cout
          << "Starting consolidation of deletes and compacting data.....";
      index.consolidate(paras);
      greator::cout << "completed in " << del_timer.elapsed() / 1000000.0
                    << "sec." << std::endl;

      greator::cout << "Search post deletion....." << std::endl;
      search_kernel(query, query_num, query_aligned_dim, recall_at, Lvec, index,
...
#pragma omp parallel for
      ...
      greator::cout << "Re-incremental time: " << timer.elapsed() / 1000
                    << "ms\n";
      search_kernel(query, query_num, query_aligned_dim, recall_at, Lvec, index,
                    truthset_file);
    }
