...
template <typename T> int search_memory_index(int argc, char **argv) {
  ...
  greator::Metric m = (distance_metric == "cosine" ? greator::Metric::COSINE
                                                   : greator::Metric::L2);

  if (distance_metric != "l2" && m == greator::Metric::L2) {
  ...
  std::cout << "Running search on type " << typeid(T).name()
            ...
            << " similarity metric: "
            << (m == greator::Metric::COSINE ? "cosine" : "l2")
  ...
  greator::Index<T, uint32_t> index(m, query_dim, max_points, dynamic_index,
  ...
  greator::cout << "Index loaded" << std::endl;
