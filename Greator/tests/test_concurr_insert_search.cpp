...
template <typename T, typename TagT>
void insert_kernel(T *data_load, greator::Index<T, TagT> &index,
                   greator::Parameters &parameters, size_t num_points,
                   size_t aligned_dim, unsigned start) {
  greator::cout << "Insertion thread" << std::endl;
  greator::Timer timer;
...
#pragma omp parallel for num_threads(16)
  ...
  greator::cout << "Insert time : " << time_secs << " s\n"
...
int build_incremental_index(const std::string &data_path, const unsigned L,
  ...
  typedef int TagT;
  greator::Index<T, TagT> index(greator::L2, dim, num_points, 1, true, false);
  ...
  greator::cout << "Saved batch build" << std::endl;
  T *query = NULL;
