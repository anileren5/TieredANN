...
int build_incremental_index(const std::string &data_path, const unsigned L,
  ...
  greator::Index<T, TagT> index(greator::L2, dim, num_points, num_frozen, true,
                                true);
  greator::cout << "num_points = " << num_points;
  {
    ...
    greator::cout << "Index build time: " << timer.elapsed() / 1000 << "ms\n";
  }
  ...
  for (unsigned i = 0; i < rounds; i++) {
    greator::cout << i << std::endl << std::endl;
    ...
    greator::cout << "Inserting " << insert_size << " points" << std::endl;
    ...
    greator::cout << "Insertion time " << insert_timer.elapsed() / 1000
                  << "ms\n";
    ...
    greator::cout << "Number of points in the index post insertion "
    ...
    greator::cout << "Deleting " << delete_size << " points from the index"
                  << std::endl;
    ...
    greator::cout << "Delete time in this phase " << del_timer.elapsed() / 1000
                  << "ms\n";
    deleted_tags.clear();
    inserted_tags.clear();
    ...
    greator::cout << "Number of points in the graph currently = "
                  << num_curr_pts << std::endl;
  }

  {
    ...
    greator::cout << "Incremental time: " << timer.elapsed() / 1000 << "ms\n";
...
int main(int argc, char **argv) {
  ...
  if (std::string(argv[1]) == std::string("int8"))
    ...
    greator::cout << "Unsupported type. Use float/int8/uint8" << std::endl;
}
