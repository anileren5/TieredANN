...
void dump_to_disk(const T *all_pts, const uint64_t ndims,
  ...
  greator::cout << "# points : " << tags.size() << "\n";
  greator::cout << "Tag file : " << tag_filename << "\n";
  greator::cout << "Data file : " << data_filename << "\n";
  ...
  //  tag_writer.close();
  greator::save_bin<TagT>(tag_filename, new_tags, tags.size(), 1);
  greator::save_bin<T>(data_filename, new_data, tags.size(), ndims);
  delete new_data;
  delete new_tags;
}
...
void run(const uint32_t base_count, const uint32_t num_mem_indices,
  ...
  uint64_t npts, ndims;
  greator::load_bin(in_file, all_points, npts, ndims);
  greator::cout << "Loaded " << npts << " pts x " << ndims << " dims\n";
  //  assert(npts >= base_count + num_mem_indices);

  std::vector<uint32_t> tags(npts);
  std::iota(tags.begin(), tags.end(), 0);

  greator::cout << "Base Index : choosing " << base_count << " points\n";
  ...
  while (count++ < num_cycles) {
    ...
    greator::cout << "Merge program will insert " << insert_vec.size()
    ...
    for (uint64_t i = 0; i < num_mem_indices; i++) {
      ...
      greator::cout << "Index #" << i + 1 << " : choosing "
    ...
    // write mem
    for (uint64_t i = 0; i < num_mem_indices; i++) {
      greator::cout << "Dumping mem set #" << i + 1 << "\n";
...
int main(int argc, char **argv) {
  if (argc != 10) {
    greator::cout << "Correct usage: " << argv[0]
  ...
  if (index_type == std::string("float")) {
    ...
    greator::cout << "Unsupported type : " << index_type << "\n";
  }
  greator::cout << "Exiting\n";
}
