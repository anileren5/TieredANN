...
int main(int argc, char **argv) {
  if (argc != 4) {
    greator::cout << argv[0] << " <ground_truth_bin> <our_results_bin>  <r> "
                  << std::endl;
    return -1;
  }
  ...
  size_t dim_gs;
  size_t dim_or;
  greator::load_truthset(argv[1], gold_std, gs_dist, points_num_gs, dim_gs);
  greator::load_truthset(argv[2], our_results, or_dist, points_num_or, dim_or);
  ...
  if ((dim_or < recall_at) || (recall_at > dim_gs)) {
    greator::cout << "ground truth has size " << dim_gs << "; our set has "
                  << dim_or << " points. Asking for recall " << recall_at
                  << std::endl;
    return -1;
  }
  greator::cout << "Calculating recall@" << recall_at << std::endl;
  float recall_val = (float)greator::calculate_recall(
      (_u32)points_num, gold_std, gs_dist, (_u32)dim_gs, our_results,
      (_u32)dim_or, recall_at);

  //  double avg_recall = (recall*1.0)/(points_num*1.0);
  greator::cout << "Avg. recall@" << recall_at << " is " << recall_val << "\n";
}
