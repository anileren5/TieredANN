...
uint8_t *getData(const char *infileName, uint8_t dataTypeSizeInBytes,
  ...
  greator::cout << infileName << ": count: " << count
...
void concat_files(int argc, char **argv) {
  ...
  if (dimension1 != dimension2) {
    greator::cout << "Error! Cannot combine vectors of differing dimensions ("
  ...
  greator::cout << "Concatenated " << (count1 + count2) << " " << dimension1
...
void uniquePtrAssignment() {
  ...
  for (int i = 0; i < 30; i++) {
    greator::cout << data[i] << " ";
    if (data[i] - i > 0.001) {
      greator::cout << data[i] << "," << i << " screwed " << std::endl;
      break;
    }
  }
  greator::cout << std::endl;
  greator::cout << "safe." << std::endl;
}
...
void compareDistanceComputationsInt() {
  ...
  if (!neg) {
    greator::cout << "No negative numbers in test. " << std::endl;
  }
...

  {
    ...
    greator::AVXDistanceL2Int8 dist;
  ...
  {
    greator::DistanceCosineInt8 dist;
    float dist1 = dist.compare(vec1, vec2, 16);
...
void compareDistanceComputationsFloat() {
  ...
  {
    greator::AVXDistanceL2Float dist;
  ...
  {
    greator::DistanceCosineFloat dist;
    float dist1 = dist.compare(a, b, 16);
...
void testStreamBufImpl() {
  ...
  greator::cout << "Printing with diskann::cout" << std::endl;
#pragma omp parallel for schedule(dynamic, 64)
  for (int i = 0; i < v.size(); i++) {
    greator::cout << std::to_string(i) + ",";
    if (i != 0 && i % 10 == 0) {
      greator::cout << std::endl;
    }
  }
...
void testMemBufImpl(int argc, char **argv) {
  ...
  greator::cout << "Read " << size << " bytes from file" << std::endl;
  ...
  greator::cout << int1 << "," << int2 << "," << f2 << std::endl;
  for (int i = 0; i < F_ARRAY_LEN; i++) {
    greator::cout << fs1[i] << ",";
  }
  greator::cout << std::endl;
  for (int i = 0; i < I_ARRAY_LEN; i++) {
    greator::cout << std::to_string(arr1[i]) << ",";
  }
  greator::cout << std::endl;
...
void testSubstringImpl(int argc, char **argv) {
  ...
  greator::cout << "Common substring at 0 is:" << s1.substr(0, index);
}
...
int main(int argc, char **argv) {
  ...
  case 7:
    ...
    break;
  default:
    greator::cout << "Don't know what to do with mode parameter: " << argv[1]
                  << std::endl;
  }
}
