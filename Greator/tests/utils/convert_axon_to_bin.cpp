...
uint32_t getRowCount(const char *infile) {
  ...
  while (std::getline(fin, line)) {
    if (line.length() != 0) {
      count++;
      if (count % 10000 == 0) {
        greator::cout << "Counted " << count << " lines." << std::endl;
      }
    }
  }
...
void writeBinaryVec(const char *outfile, uint32_t numDimensions,
  ...
  greator::cout << "Writing  " << numPoints
...
//    diskann::cout << "Writing  " << numPoints
//              << " records of dimension: " << numDimensions
//              << " to file: " << outfile << "...";
//
//    std::ofstream fout(outfile, std::ios::binary);
...
void sanityCheck(const char *origfile, const char *binfile,
  ...
  greator::cout << "Checking first " << vectorsToCheck
  ...
  if (mismatchCount == 0) {
    greator::cout << "Successfully verified byte-byte conversion." << std::endl;
  } else {
    greator::cout << "Conversion failed!!! Found " << mismatchCount
                  << " mismatches in vector elements" << std::endl;
  }
  greator::cout << "Successfully verified binary conversion." << std::endl;
}
  ...
  while (std::getline(fin, line)) {
    ...
    if (vector.size() != 0) {
      if (vector.size() != numDimensions) {
        ...
        throw greator::ANNException(stream.str(), -1);
      }
      allVecs.push_back(vector);
      if (allVecs.size() % 10000 == 0) {
        greator::cout << "Processed " << allVecs.size() << " records."
                      << std::endl;
      }
    ...
    //    diskann::cout << "Processed " << index / numDimensions << "
    //    records."
    //              << std::endl;
    //}
  }
...
int main(int argc, char **argv) {
  ...
  greator::cout << "Converting (" << argv[1] << ") axon file: " << argv[2]
