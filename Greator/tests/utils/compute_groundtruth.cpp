...
#pragma omp parallel for schedule(static, 65536)
  for (int64_t d = 0; d < num_points; ++d)
    points_l2sq[d] = cblas_sdot(dim, matrix + (ptrdiff_t)d * (ptrdiff_t)dim, 1,
    ...
    float *ones_vec = NULL) // Scratchspace of num_data size and init to 1.0
{
  ...
  cblas_sgemm(CblasColMajor, CblasNoTrans, CblasTrans, npoints, nqueries, 1,
              (float)1.0, points_l2sq, npoints, ones_vec, nqueries, (float)1.0,
              dist_matrix, npoints);
  cblas_sgemm(CblasColMajor, CblasNoTrans, CblasTrans, npoints, nqueries, 1,
              (float)1.0, ones_vec, npoints, queries_l2sq, nqueries, (float)1.0,
              dist_matrix, npoints);
  if (ones_vec_alloc)
    delete[] ones_vec;
}
               ...
               const float *const queries) // queries in Col major
{
  ...
  for (_u64 b = 0; b < div_round_up(nqueries, q_batch_size); ++b) {
    ...
    distsq_to_points(dim, dist_matrix, npoints, points, points_l2sq, q_e - q_b,
                     ...
                     queries_l2sq + q_b);
    greator::cout << "Computed distances for queries: [" << q_b << "," << q_e
                  << ")" << std::endl;
    ...
    for (long long q = q_b; q < q_e; q++) {
      ...
      for (_u64 p = 0; p < k; p++)
        point_dist.emplace(p,
                           dist_matrix[(ptrdiff_t)p + (ptrdiff_t)(q - q_b) *
                                                          (ptrdiff_t)npoints]);
      ...
      assert(std::is_sorted(dist_closest_points + (ptrdiff_t)q * (ptrdiff_t)k,
                            dist_closest_points +
                                (ptrdiff_t)(q + 1) * (ptrdiff_t)k));
    }
    greator::cout << "Computed exact k-NN for queries: [" << q_b << "," << q_e
                  << ")" << std::endl;
  }
...
template <typename T> inline int get_num_parts(const char *filename) {
  ...
  greator::cout << "Reading bin file " << filename << " ...\n";
  int npts_i32, ndims_i32;
  reader.read((char *)&npts_i32, sizeof(int));
  reader.read((char *)&ndims_i32, sizeof(int));
  greator::cout << "#pts = " << npts_i32 << ", #dims = " << ndims_i32
                << std::endl;
  reader.close();
  uint32_t num_parts =
      (npts_i32 % PARTSIZE) == 0
  ...
  greator::cout << "Number of parts: " << num_parts << std::endl;
  return num_parts;
}
...
inline void load_bin_as_float(const char *filename, float *&data, size_t &npts,
  ...
  greator::cout << "#pts in part = " << npts << ", #dims = " << ndims
  ...
  greator::cout << "Finished reading part of the bin file." << std::endl;
  reader.close();
...
#pragma omp parallel for schedule(dynamic, 32768)
  ...
  delete[] data_T;
  greator::cout << "Finished converting part data to float." << std::endl;
}
...
inline void save_bin(const std::string filename, T *data, size_t npts,
  ...
  greator::cout << "Writing bin: " << filename << "\n";
...
inline void save_groundtruth_as_one_file(const std::string filename,
  ...
  writer.close();
  greator::cout << "Finished writing truthset" << std::endl;
}
...
template <typename T> int aux_main(int argc, char **argv) {
  ...
  for (int p = 0; p < num_parts; p++) {
    ...
    greator::aligned_free(base_data);
  }
  ...
  if (argc == 7) {
    ...
    greator::load_bin(tag_file, all_tags, tag_pts, tag_dim);

    greator::cout << "Loaded tags for " << tag_pts << " points.\n";
  ...
  save_groundtruth_as_one_file(gt_file, closest_points, dist_closest_points,
                               nqueries, k, tags);
  greator::aligned_free(query_data);
