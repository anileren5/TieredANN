...
void seed_iter(tsl::robin_set<uint32_t> &active_set,
  ...
  greator::save_bin<TagT>(inserted_tags_file, tag_data, insert_vec.size(), 1);
  delete[] tag_data;
  ...
  greator::cout << "ITER: end = " << active_set.size() << ", "
                << inactive_set.size() << "\n";
#ifndef _WINDOWS
...
void search_disk_index(const std::string &index_prefix_path,
  ...
  std::unique_ptr<greator::PQFlashIndex<T, uint32_t>> _pFlashIndex(
      new greator::PQFlashIndex<T, uint32_t>(greator::Metric::L2, reader,
...
#pragma omp parallel for schedule(dynamic, 1) // num_threads(1)
  ...
  for (uint32_t i = 0; i < query_num; i++) {
    ...
    float query_recall = compute_active_recall(result_tags, (uint32_t)recall_at,
                                               gs_tags, gt_dim, inactive_tags);
...
template <typename T, typename TagT = uint32_t>
void search_kernel(greator::MergeInsert<T> &merge_insert,
...
#pragma omp parallel for num_threads(NUM_SEARCH_THREADS)
    ...
    // compute mean recall, IOs
    float mean_recall = 0.0f;
    mean_recall = greator::calculate_recall(
...
template <typename T, typename TagT = uint32_t>
void insertion_kernel(greator::MergeInsert<T> &merge_insert,
  ...
  greator::load_aligned_bin<T>(mem_pts_file, data_insert, npts, ndim,
                               aligned_dim);
  size_t tag_num, tag_dim;
  TagT *tag_data;
  greator::load_bin<TagT>(mem_tags_file, tag_data, tag_num, tag_dim);
  if (tag_num != npts) {
...
template <typename T, typename TagT = uint32_t>
void deletion_kernel(greator::MergeInsert<T> &merge_insert,
...
template <typename T> void merge_kernel(greator::MergeInsert<T> &merge_insert) {
  merge_insert.final_merge();
}

template <typename T, typename TagT = uint32_t>
void run_iter(greator::MergeInsert<T> &merge_insert,
...
template <typename T, typename TagT = uint32_t>
void run_single_iter(greator::MergeInsert<T> &merge_insert,
                     const std::string &base_prefix,
                     const std::string &merge_prefix,
                     const std::string &mem_prefix,
                     tsl::robin_set<uint32_t> &active_set,
                     tsl::robin_set<uint32_t> &inactive_set,
                     greator::Distance<T> *dist_cmp) {
  // files for mem-DiskANN
  std::string mem_pts_file = mem_prefix + ".data_orig";
  std::string mem_tags_file = mem_prefix + ".tags_orig";
  if (::_insertions_done.load() && ::_del_done.load()) {
...
void run_all_iters(std::string base_prefix, std::string merge_prefix,
  ...
  greator::get_bin_metadata(data_file, npts, ndims);
  ...
  if (::save_index_as_one_file) {
    uint64_t *metadata;
    size_t nr, nc;
    greator::load_bin<uint64_t>(active_tags_file, metadata, nr, nc);
    std::cout << nr << " " << nc << std::endl;
    greator::load_bin<TagT>(active_tags_file, tag_data, tag_num, tag_dim,
                            metadata[7]);
  ...
  greator::MergeInsert<T, TagT> merge_insert(
