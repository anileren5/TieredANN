...
template <typename T> using IncrIndex = greator::DiskANNIncrementalIndex<T>;

template <typename T> class InMemoryIndexParallelTest {
public:
...
template <typename T>
std::shared_ptr<IncrIndex<T>>
initialCreateInMemoryIndex(T *data, _u32 npts, _u32 ndims, _u32 pointsPerIndex,
...
void runIncrementalIntfTest(const std::string &basefile,
  ...
  T *data;
  size_t npts, ndim;
  greator::load_bin<basefile, data, npts, ndim, 0);
