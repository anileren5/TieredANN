...
int build_in_memory_index(const std::string &data_path,
                          ...
                          bool single_file_index, greator::Metric distMetric) {
  ...
  greator::get_bin_metadata(data_path, data_num, data_dim);
  greator::cout << "Building in-memory index with parameters: data_file: "
                << data_path << "tags file: " << tags_file << " R: " << R
                << " L: " << L << " alpha: " << alpha
                << " index_path: " << save_path << " #threads: " << num_threads
                << ", using distance metric: "
                << (distMetric == greator::Metric::COSINE ? "cosine " : "l2 ");

  typedef int TagT;

  // TODO: Should not hard code enable tags.
  greator::Index<T, TagT> index(distMetric, data_dim, data_num, dynamic_index,
                                single_file_index,
  ...
  if (dynamic_index) {
    ...
    greator::cout << "Indexing time: " << diff.count() << "\n";
  } else {
    std::vector<TagT> tags(data_num);
    std::iota(tags.begin(), tags.end(), 0);
...
int main(int argc, char **argv) {
  ...
  enum greator::Metric distMetric =
      dist_metric_str == "cosine"
          ? greator::Metric::COSINE
          : greator::Metric::L2; // set to l2 even if something else is chosen

  if (dist_metric_str != "l2" && distMetric == greator::Metric::L2) {
