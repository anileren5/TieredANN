#!/usr/bin/env python3
"""
Python version of qvcache_split_search_bruteforce_backend.py using Milvus backend

This script allows running the same test as the C++ version but using
a Python-implemented Milvus backend.
"""
...
def experiment_split(
    ...
    """Run the split search experiment with Milvus backend."""
    # Create QVCache with Milvus backend
    qvcache = qvc.QVCache(
        data_path=data_path,
...
def main():
    parser = argparse.ArgumentParser(description="QVCache split search experiment with Milvus backend")
    ...
    parser.add_argument("--collection_name", type=str, default="vectors",
                       help="Milvus collection name (default: vectors)")
    parser.add_argument("--milvus_host", type=str, default="localhost",
                       help="Milvus service host (default: localhost)")
    parser.add_argument("--milvus_port", type=int, default=19530,
                       help="Milvus service port (default: 19530)")
    ...
    # Create Milvus backend (assumes index already exists)
    print(f"\nConnecting to Milvus at {args.milvus_host}:{args.milvus_port}...")
    backend = MilvusBackend(
        collection_name=args.collection_name,
        dimension=dimension,
        milvus_host=args.milvus_host,
    ...
    experiment_split(
        ...
        args.search_strategy, backend, args.collection_name, args.milvus_host, args.milvus_port
    )
