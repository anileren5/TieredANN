...
PYBIND11_MODULE(tieredann, m) {
    ...
    m.doc() = "TieredANN Python bindings";

    // Bind SearchStrategy enum
    py::enum_<TieredIndex<float>::SearchStrategy>(m, "SearchStrategy")
        .value("SEQUENTIAL_LRU_STOP_FIRST_HIT", TieredIndex<float>::SearchStrategy::SEQUENTIAL_LRU_STOP_FIRST_HIT)
        .value("SEQUENTIAL_LRU_ADAPTIVE", TieredIndex<float>::SearchStrategy::SEQUENTIAL_LRU_ADAPTIVE)
        .value("SEQUENTIAL_ALL", TieredIndex<float>::SearchStrategy::SEQUENTIAL_ALL)
        .value("PARALLEL", TieredIndex<float>::SearchStrategy::PARALLEL);

    // Bind TieredIndex for float
    py::class_<TieredIndex<float>>(m, "TieredIndex")
        .def(py::init([](const std::string& data_path,
            ...
            if (python_backend.is_none()) {
                return std::make_unique<TieredIndex<float>>(
            ...
            "Construct TieredIndex with optional Python backend")
        .def("search", [](TieredIndex<float>& index, 
        ...
        .def("set_search_strategy", &TieredIndex<float>::set_search_strategy)
        .def("get_search_strategy", &TieredIndex<float>::get_search_strategy)
        .def("enable_adaptive_strategy", &TieredIndex<float>::enable_adaptive_strategy)
        .def("set_hit_ratio_window_size", &TieredIndex<float>::set_hit_ratio_window_size)
        .def("set_hit_ratio_threshold", &TieredIndex<float>::set_hit_ratio_threshold)
        .def("get_number_of_vectors_in_memory_index", &TieredIndex<float>::get_number_of_vectors_in_memory_index)
        .def("get_number_of_max_points_in_memory_index", &TieredIndex<float>::get_number_of_max_points_in_memory_index)
        .def("get_number_of_mini_indexes", &TieredIndex<float>::get_number_of_mini_indexes)
        .def("get_index_vector_count", &TieredIndex<float>::get_index_vector_count)
        .def("get_number_of_active_pca_regions", &TieredIndex<float>::get_number_of_active_pca_regions);

    // Helper function to create TieredIndex with Python backend
    m.def("create_tiered_index_with_python_backend", 
        ...
        "Create TieredIndex with a Python backend implementation");
