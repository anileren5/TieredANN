#!/usr/bin/env python3
"""
Windowed backend-only benchmark using Qdrant backend (without QVCache)

This script benchmarks the Qdrant backend directly, processing queries in windows.
"""
...
def experiment_benchmark(
    ...
    """Run the windowed backend-only benchmark experiment with Qdrant backend."""
    # Load ground truth
...
def main():
    parser = argparse.ArgumentParser(description="Windowed backend-only benchmark experiment with Qdrant backend")
