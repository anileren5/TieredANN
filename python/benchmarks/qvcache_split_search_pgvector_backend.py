#!/usr/bin/env python3
"""
Python version of qvcache_split_search_bruteforce_backend.py using pgvector backend

This script allows running the same test as the C++ version but using
a Python-implemented pgvector backend.
"""
...
def main():
    parser = argparse.ArgumentParser(description="QVCache split search experiment with pgvector backend")
    ...
    print(f"\nConnecting to PostgreSQL at {args.db_host}:{args.db_port}...")
    backend = PgVectorBackend(
        table_name=args.table_name,
        dimension=dimension,
        db_host=args.db_host,
